...
tokens = [
    'IDENTIFIER', 'NUMBER',
    # Delimiters
    'LPAREN', 'RPAREN', 'LCURLY', 'RCURLY', 'SEMICOLON', 'COMMA', 'POINT', 
    'APOS',
    # Operators
...
def gettype(expression):
    if (isinstance(expression, int) and type(expression) == int) or expression == "int":
        return "int"
    if (isinstance(expression, float) and type(expression) == float) or expression == "float":
        return "float"
    if (isinstance(expression, bool) and type(expression) == bool) or expression == "boolean":
        return "boolean"
    if (isinstance(expression, str) and type(expression) == str) or expression == "str":
        return "str"
    return "char"

def is_valid_params(id, args):
    if functions[id]["parameters"] == None:
        return args == [None]
    if len(functions[id]["parameters"]) != len(args):
        print(f"WARNING: Function parameters don't match ones found in definition.")
        return False
    for i in range(len(args)):
        temp = functions[id]["parameters"][i][1]
        if temp != gettype(variables[args[i]]):
            print(f"WARNING: Argument '{args[i]}' is not the same type as '{temp}'.")
            return False
    return True
...
def p_while_statement(p):
    ...
    whiles[p[3]] = {"expression" : p[3], "statements" : p[6]}
    p[0] = {"expression" : p[3], "statements" : p[6]}
...
def p_function(p):
    ...
    if len(p) == 6:
        ...
        functions[p[2]] = {"parameters": p[4], "return_type": p[6], "body": p[8]}
        funcreturns.append(p[6])
        p[0] = functions[p[2]]
